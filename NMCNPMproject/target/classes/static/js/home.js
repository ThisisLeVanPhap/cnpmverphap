...
async function takeRateByUser() {
  try {
    const response = await fetch(
      `http://localhost:8080/api/rates/getrate/${infoUser.userID}`
    );
      ...
      if (data) {
        ...
        Array.from(stars).forEach((star, index) => {
          ...
          star.addEventListener("click", async () => {
            ...
            try {
              const response = await fetch(
                `http://localhost:8080/api/rates/create`,
                {
                  method: "POST",
...
async function addContentCmtByBookID(bookID) {
  try {
    const response = await fetch(
      `http://localhost:8080/api/comments/getcmt/${bookID}`
    );
    if (!response.ok) {
      throw new Error(`Error, Status: ${response.status}`);
    }
      ...
      let sectionCmt = `
                <li id="cmt-id-${comment.commentID}">
                     <div class="area-content-cmt">
                        <span class="avatar">
                            <img class="user-pic-cmt" src="${
                              comment.pathPicture
                            }">
                        </span>
                        <div class="cmt-content">
                            <p class="usernameF">${comment.username}
                                <span class="time-cmt" data-lastUpdate="${timeComment.format(
                                  "DD/MM/YYYY HH:mm:ss"
                                )}">${timeAgo}</span>
                            </p>
                            <p id="content-cmt-id-${comment.commentID}">${
        comment.comment
      }</p>
                        </div>
                        <span class="chevron-icon" onclick="showMenuActions(${
                          comment.commentID
                        })">
                        ...
                        <button id="menu-${
                          comment.commentID
                        }"class="delete-edit-cmt">
                            <ul>
                                <li onclick="confirmDelete(${
                                  comment.commentID
                                })">Xóa bình luận</li>
                                <li onclick="showButtonEdit(${
                                  comment.commentID
                                })">Sửa bình luận</li>
                            </ul>
                        </button>
    ...
    const updateAllTimeAgo = () => {
      ...
      timeSpans.forEach((timeSpan) => {
        const timeComment = moment(
          timeSpan.getAttribute("data-lastUpdate"),
          "DD/MM/YYYY HH:mm:ss"
        );
...
async function postComment() {
  const comment = {
    ...
    userID: infoUser.userID,
    bookID: parseInt(
      document.getElementById("book-id").textContent.replace("ID: ", "")
    ),
  };
  try {
...
function showMenuActions(commentID) {
  Array.from(document.querySelectorAll(".delete-edit-cmt")).forEach(
    (menuCmt) => {
...
function showButtonEdit(commentID) {
  const contentComment = document.getElementById(
    `content-cmt-id-${commentID}`
  ).textContent;
...
async function editComment(commentID) {
  ...
  try {
    ...
    const timeAgo = timeComment.fromNow();
    document.getElementById(`content-cmt-id-${commentID}`).innerText =
      dataToSend.contentEdited;
  } catch (error) {
    console.error(error);
  }
}
